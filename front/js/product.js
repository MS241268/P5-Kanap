...
const urlApi = `http://192.168.1.182:3000/api/products/${idCurrentPageProduct}`; //Chargement des caractéristiques du produit depuis l'API
    ...
    response
      ...
      .then((dataProduct) => {
        ...
        document.querySelector(
          '.item__img'
        ).innerHTML = `<img src=${dataProduct.imageUrl} alt="${dataProduct.altTxt}">`; //Implémentation de l'image du produit dans la page produit.html
        ...
        document.querySelector('#description').innerText =
          dataProduct.description; //Implémentation de la description du produit dans la page product.html

        //Gestion des options de couleurs du produit sélectionné
        for (let colorsProduct of dataProduct.colors) {
          // Boucle pour connaître les couleurs du produit sélectionné
...
addBasket.addEventListener('click', () => {
  ...
  const selectedColorProduct =
    listColorProduct.options[listColorProduct.selectedIndex].text;
  ...
  if (quantityProduct > 100 || quantityProduct < 1) {
    alert(
      'La quantité pour ce produit doit être comprise entre 1 et 100 maximum'
    );
  }

  if (
    selectedColorProduct != '--SVP, choisissez une couleur --' &&
    quantityProduct > 0 &&
    quantityProduct <= 100
  ) {
    ...
    if (localStorage.getItem('basketProducts') == null) {
      //Vérification si le localStorage est vide
      ...
      let foundColorProduct = arrayProductsLs.find(
        (c) => c.color == recapProduct.color
      ); //Vérification si la couleur produit existe dans le localStorage
      if (foundIdProduct != undefined && foundColorProduct != undefined) {
        //Condition si le produit de la même couleur existe dans le localStorage
        for (let i = 0; i < arrayProductsLs.length; i++) {
          //Boucle pour séléctionner le bon Id produit avec la bonne couleur
          if (
            arrayProductsLs[i].id == recapProduct.id &&
            arrayProductsLs[i].color == recapProduct.color
          ) {
            //Vérification du bon Id produit et de la bonne couleur
            arrayProductsLs[i].quantity =
              arrayProductsLs[i].quantity + recapProduct.quantity;
            ...
            localStorage.setItem(
              'basketProducts',
              JSON.stringify(arrayProductsLs)
            ); //Injection du produit avec la nouvelle quantité
            alert('Produit(s) ajouté(s) dans votre panier');
          }
        }
      } else {
        //Condition si nouveau produit non présent dans le localStorage
